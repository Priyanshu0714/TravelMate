...
function wrap_matching_traveller(data) {
  ...
  data.records.forEach((temp, index) => {
    document.getElementById("create-matching-travellers").innerHTML +=
      `<div  class="h-auto w-[400px] border border-gray-400 bg-gray-200 flex gap-5 flex-col rounded-lg p-4">
  ...
  <button name="connect_button" value="${temp.current_user_id}" id="matching-traveller-connect-${index}" class="bhpuesh w-full bg-black text-white border border-gray-400 h-9 outline-none rounded-lg mt-3">Connect</button>
</form>
            </div>
          </div>`;
  });
  ...
  // for getting the receiverID
  document
    .querySelectorAll('form[action="/connect_button"]')
    .forEach((form) => {
      form.addEventListener("submit", async (e) => {
        e.preventDefault();
        selectedReceiverId = e.target.firstElementChild.value;
        console.log(selectedReceiverId);
        chatBox.click();

        const response = await fetch(`/chatbox/${selectedReceiverId}`, {
          method: "GET",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
        });
        if (response.ok) {
          const { findmessage, senderId } = await response.json();
          console.log(`this is the user id ${senderId}`);
          findmessage.forEach((d) => {
            console.log(d);
            if (senderId == d.senderId) {
              chattingArea.innerHTML += `<div class="float-right clear-both border border-blue-400 p-2 rounded-lg max-w-[45%] flex justify-end">${d.message}</div>`;
            } else {
              chattingArea.innerHTML += `<div class="float-left clear-both border border-blue-400 p-2 rounded-lg max-w-[45%] flex justify-end">${d.message}</div>`;
            }
          });
        } else {
          chattingArea.innerHTML += `<div class="float-right clear-both border border-blue-400 p-2 rounded-lg max-w-[45%] flex justify-end text-red-400">unable to load messages</div>`;
        }
      });
    });
